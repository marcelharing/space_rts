...
class Base {
  constructor(owner, x, y, game) {
    ...
    this.iron = owner === 'player' ? 120 : 0;
    this.gold = owner === 'player' ? 70 : 0;
    this.diamond = 0;
    this.queue = [];
    this.active = null;
...
class Unit {
  ...
  update(dt) {
    ...
    if (!this.moveOverride && enemy && distance(this, enemy) <= range + getTargetRadius(enemy)) {
      this.moveTarget = null;
      if (this.cooldown <= 0) {
...
this.stormHazardTimer = 0;
  ...
  createResourceMeteor(index = 0) {
    ...
    const iron = resourceType === 'iron' ? rand(70, 120) * 1.15 * resourceMultiplier : 0;
    const gold = resourceType === 'gold' ? rand(40, 75) * 0.6 * resourceMultiplier : 0;
...
if (this.isStormRetreatActive()) {
		...
		if (aiWorkers < desiredWorkers && aiBase.enqueueUnit('worker')) return;

      const pressure = difficulty.aggression;
      const chooseRanged = Math.random() < 0.35 + pressure * 0.25;
      if (aiBase.canAffordUnit(chooseRanged ? 'ranged' : 'melee')) {
